...
func TestDurationMetric(t *testing.T) {
	...
	count, avg, minDuration, maxDuration := d.Stats()
	...
	if minDuration != 50*time.Millisecond {
		t.Errorf("expected min 50ms, got %v", minDuration)
	}

	if maxDuration != 300*time.Millisecond {
		t.Errorf("expected max 300ms, got %v", maxDuration)
	}
}
